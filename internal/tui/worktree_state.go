...
func loadWorktreeRows(repoRoot string, cfg *config.ProjectConfig) ([]worktreeRow, error) {
	...
	for _, wt := range wts {
		...
		row := worktreeRow{
			...
			State:       worktreeStateFor(wt.Path),
		}
		if row.Branch == "" {
...
// "dirty (N files)", "unpushed", "clean", or an error description if checks failed.
func worktreeStateFor(path string) string {
	dirty, err := gitutil.DirtyFiles(path)
	if err != nil {
		return "err: " + err.Error()
	}
	if len(dirty) > 0 {
		return fmt.Sprintf("dirty (%d files)", len(dirty))
	}
